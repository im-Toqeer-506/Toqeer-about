...
If you only want to run the portfolio locally, use this short version:
...
If you want GitHub profile data and pinned repositories to appear, set `REACT_APP_GITHUB_TOKEN`, `GITHUB_USERNAME`, and `USE_GITHUB_DATA="true"` in `.env`.

If you want Medium blog posts to show up, set `MEDIUM_USERNAME` in `.env`.

If you do not want external data, leave the GitHub and Medium values empty and focus on editing `src/portfolio.js`.
...
This project keeps contributor credit in text form only, without the avatar grid.
